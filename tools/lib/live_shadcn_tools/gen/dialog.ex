defmodule LiveShadcnTools.Gen.Dialog do
  ...
  defp moduledoc(spec) do
    """
      @moduledoc \"\"\"
      #{spec["name"] |> String.replace("-", " ") |> String.capitalize()}. #{summary(spec)}

      Generated by `mix ui.gen` from `registry/spec/#{spec["name"]}.json`. Every
  ...
  defp summary(spec), do: get_in(spec, ["primitives", "#{spec["name"]}.Root", "summary"]) || ""
end
