  ...
  def module(spec, opts) do
    ...
    #{moduledoc(spec)}
      ...
      def calendar(assigns) do
        ...
        ~H\"\"\"
        <section data-slot="calendar" style="min-width: 157px" class={["cn-calendar group/calendar bg-background w-fit rdp-root", @class]} {@rest}>
          ...
          <table style="margin-bottom: 13px" class="w-full border-collapse rdp-month_grid" role="grid" aria-label={Calendar.strftime(@month, "%B %Y")}>
            <thead>
              <tr class="flex rdp-weekdays">
                <th :for={day <- weekday_names(@week_starts_on)} style="width: 19px" class="flex-1 rounded-(--cell-radius) text-[0.8rem] font-normal text-muted-foreground select-none rdp-weekday" scope="col">{day}</th>
              </tr>
            </thead>
            <tbody>
